...
const NodeComponent: React.FC<NodeProps> = ({ 
  ...
  const handleDownload = (e: React.MouseEvent) => { e.stopPropagation(); const a = document.createElement('a'); a.href = node.data.image || videoBlobUrl || node.data.audioUri || ''; a.download = `netopstudio-${Date.now()}`; document.body.appendChild(a); a.click(); document.body.removeChild(a); };
  ...
  const renderBottomPanel = () => {
     ...
     if (node.type === NodeType.VIDEO_GENERATOR) {
        models = [
         ...
         models = [{l: 'Gemini 3 Pro', v: 'gemini-3-pro-preview'}, {l: 'Gemini 2.5 Flash', v: 'gemini-2.5-flash'}];
        ...
        models = [
            ...
            {l: 'Gemini 3 Pro Image', v: 'gemini-3-pro-image-preview'}
        ];
     }
