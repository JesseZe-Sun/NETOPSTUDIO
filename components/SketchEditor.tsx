    ...
    const handleGenerate = async () => {
        ...
        try {
            if (activeMode === 'pose') {
                ...
                // 1. Generate Line Art from Gemini 3 Pro
                const posePrompt = `
                ...
                const res = await generateImageFromText(posePrompt, 'gemini-3-pro-preview', [], { aspectRatio: '16:9', count: 1 });
                const imgUrl = res[0];
            ...
            <div className="h-20 bg-[#1c1c1e] border-t border-white/10 flex items-center px-8 gap-4">
                ...
                <div className="flex items-center gap-3">
                    <div className="h-11 px-4 flex items-center gap-2 bg-black/30 border border-white/10 rounded-xl text-xs text-slate-300 font-medium">
                        <span>{activeMode === 'pose' ? 'Gemini 3 Pro (Pose)' : activeMode === 'video' ? 'Veo 3.1 Fast' : 'Gemini 3 Pro'}</span>
