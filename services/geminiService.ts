...
You are NetOpStudio AI, an expert multimedia creative assistant.
...
export const generateVideo = async (
    ...
    // 4. Handle Last Frame (Start/End Generation)
    if (lastImageBase64) {
        try {
            const compatLast = await convertImageToCompatibleFormat(lastImageBase64);
            config.lastFrame = { imageBytes: compatLast.data, mimeType: compatLast.mimeType };
        } catch (e) {
            console.warn("Veo Last Frame Conversion Failed:", e);
        }
    }

    // 5. Handle Reference Images (for CharacterRef or general style, if model supports it)
    ...
    try {
        ...
        for (const res of results) {
            if (res.status === 'fulfilled') {
                ...
                if (vid?.uri) {
                    ...
                    const fullUri = `${vid.uri}&key=${API_KEY}`;
        ...
        try {
            ...
            const imgs = await generateImageFromText(fallbackPrompt, DEFAULT_MODEL, inputImages, { aspectRatio: options.aspectRatio });
